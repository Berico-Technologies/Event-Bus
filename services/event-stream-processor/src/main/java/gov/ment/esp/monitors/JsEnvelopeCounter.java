...
public class JsEnvelopeCounter extends JavascriptDetector {
  ...
  private static final String BODY = "var clock = {"
          ...
          +
          // The following statement can be put in the Javascript to target
          // certain
          // envelope types or conditions
          // "    if (env.getEventType() != 'Search') return null;" +
          "    this.count++;" + "    var elapsed = clock.getElapsed(this.start);"
          ...
          + "    out.println('IE: ' + ie + ' for ' + env.getEventType());" + "    return ie;"
          + "  }" + "};" + "// test calls\n" + "searchCounter;";
