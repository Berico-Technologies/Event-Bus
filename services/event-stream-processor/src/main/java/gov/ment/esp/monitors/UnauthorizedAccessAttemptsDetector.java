...
class UnauthorizedAccessAttemptsDetector extends EventMonitor {
  ...
  public Collection<Publisher> registerPatterns(EventStreamProcessor esp) {
    ...
    String createUA =
            "insert into UnauthorizedAccesses "
                    + "select request.replyTo as userid, request, response " + "from pattern [ "
                    + ipattern + " ]";

    String createUAF =
            "insert into UnauthorizedAccessesFreq "
                    + "select userid, count(*) as freq from UnauthorizedAccesses.win:time("
                    + timeLimit + ") group by userid";
    String getUAF =
            "select * from UnauthorizedAccessesFreq.win:time(" + timeLimit + ") where freq >= "
                    + minFreq;
