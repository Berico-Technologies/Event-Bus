...
import gov.ment.esp.datastreams.StreamRange.TimeValFunc;
...
public class ActiveRange extends StreamRange {
  ...
  public String getTrendDesc() {
    ...
    int curtotal = getTotal();
    if (prev == null) {
      return item + ": No previous value";
    }
    if (curtotal == 0) {
      return item + ": No current value";
    }
    int prevtotal = prev.getTotal();
    int trendval = (curtotal - prevtotal) * 100 / prevtotal;
  ...
  public ActiveRange(StreamRange range, TimeEntry start, TimeEntry end, int periodInMillis,
  ...
  // TODO: fix bug where no events within a period doesn't cause the ActiveRange
  // to become empty
  ...
  private TimeEntry updateStart(long finaltime) {
    ...
    if (adjusted) {
      if (prev == null) {
        prev =
                new ActiveRange(range, range.start, wkgstart, periodInMillis, "PREV PERIOD "
                        + category, item);
        prev.initialize();
      } else {
