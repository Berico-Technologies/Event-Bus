package gov.ment.esp.datastreams;
...
public class ValueStreams {
  ...
  // a stream should only be created if data will immediately be added so
  // streams are
  ...
  /**
   * Add a value with timestamp to a named stream. Note: due to the way
   * ActiveRanges perform incremental calculation, this requires that timestamp
   * be monotonically increasing (i.e. you can't insert a timestamp after you've
   * inserted a later stamp. To avoid losing information if this occurs,
   * 
   * @param item
   * @param timestamp
   * @param value
  ...
  // Utility methods to allow writing time periods in comprehensible units and
  // converting them in millis
  ...
  private static String toEnglish(String item, int millis) {
    String units;
    if (millis >= days(1))
      units = makePhrase(millis, days(1), "day");
    else if (millis >= hours(1))
      units = makePhrase(millis, hours(1), "hour");
    else if (millis >= minutes(1))
      units = makePhrase(millis, minutes(1), "minute");
    else if (millis >= seconds(1))
      units = makePhrase(millis, seconds(1), "second");
    else
      units = String.format("%d millis", millis);
