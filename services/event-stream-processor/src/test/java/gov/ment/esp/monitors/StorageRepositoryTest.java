...
import gov.ment.esp.TestUtils;
import gov.ment.esp.monitors.ConsensusSearchDetector;
import gov.ment.esp.monitors.CorrelateRequestResponsesEventDetector;
import gov.ment.esp.monitors.DocumentCollectionWithHitFrequencySearchResultsDetector;
import gov.ment.esp.monitors.EventTypeDetector;
import gov.ment.esp.monitors.UnauthorizedAccessAttemptsDetector;
...
public class StorageRepositoryTest extends AbstractDetectorTest {
  ...
  public void testDynamicallyAddedMonitors() {
    ...
    // Create the monitor for signoff's, but don't add it to the repository or
    // event
    // stream processor
    ...
    // Send a signoff notification, since we have no event monitors that match
    // on that,
    // no inferred events will occur
    Envelope signoff =
            TestUtils.makeEnvelope(signoffString, null, null, "Say goodnight Gracie",
                    "George Burns");
    ...
    // add the signoff events monitor to the repository, this adds it to the
    // event
    // stream processor
    ...
    // Send a second signoff notification after adding a monitor for it; we
    // should detect
    // the second event, but not the first
    Envelope signoff2 =
            TestUtils.makeEnvelope(signoffString, null, null, "And that's the way it is.",
                    "Walter Cronkite");
    ...
    assertFalse("Checking to see if " + event + " references " + signoff, event
            .getReferencedEvents().contains(signoff));
  ...
  public void testMultiPatternRepository() {
    ...
    // no inferred events will occur
    Envelope signoff =
            TestUtils.makeEnvelope("Signoff", null, null, "Say goodnight Gracie", "Gracie Allen");
    clearAndSend(signoff);
    assertNoDetectedEvents(UnauthorizedAccessAttemptsDetector.INFERRED_TYPE);
    assertNoDetectedEvents(ConsensusSearchDetector.INFERRED_TYPE);
    assertNoDetectedEvents(CorrelateRequestResponsesEventDetector.INFERRED_TYPE);
    ...
    // Send a request (that will be granted); there should be a 'Request'
    // inferred event
    ...
    // Send a request (that will be denied); there should be a 'Request'
    // inferred event
    ...
    // Send a granted response; there should be a response inferred event and a
    // correlated
    ...
    assertNoDetectedEvents(UnauthorizedAccessAttemptsDetector.INFERRED_TYPE);
    assertNoDetectedEvents(ConsensusSearchDetector.INFERRED_TYPE);
    InferredEvent event3a =
            assertOneDetectedEvent(CorrelateRequestResponsesEventDetector.INFERRED_TYPE);
    ...
    // Send a denied response; there should be a response inferred event and a
    // correlated
    // request-response inferred event
    ...
    // Send a denied response (third for user; there should be a response
    // inferred event,
    // a correlated request-response inferred event, and an
    ...
    // Send a granted response (third for user); there should be a response
    // inferred event
    Envelope responseJB3 = TestUtils.makeAuthResponse(true, "JB3");
    clearAndSend(responseJB3);
    assertNoDetectedEvents(UnauthorizedAccessAttemptsDetector.INFERRED_TYPE);
    assertNoDetectedEvents(ConsensusSearchDetector.INFERRED_TYPE);
    InferredEvent event14a =
            assertOneDetectedEvent(CorrelateRequestResponsesEventDetector.INFERRED_TYPE);
