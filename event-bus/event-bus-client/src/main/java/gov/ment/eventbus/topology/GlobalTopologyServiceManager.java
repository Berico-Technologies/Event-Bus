...
public class GlobalTopologyServiceManager implements TopologyManager {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(GlobalTopologyServiceManager.class);
  ...
  public void start(EventManager eventManager) {
    ...
    RegisterClient registerClientEvent =
            new RegisterClient(clientName, topologyRegistry.getVersion());
    try {

      LOG.trace("Registering client {} with Global Topology Service.", clientName);

      @SuppressWarnings("unchecked")
      TopologyUpdate topologyUpdateResponseEvent =
              eventManager.getResponseTo(registerClientEvent, 5000, TopologyUpdate.class);
  ...
  public RoutingInfo getRoutingInfoForEvent(Class<?> eventType) {
    ...
    LOG.trace("Looking for route for event type [{}] in global topology service.",
  ...
  public RoutingInfo[] getRoutingInfoForNamedEventSet(String eventSetName) {
    ...
    LOG.trace("Looking for routes for event set name [{}] in global topology service.",
            eventSetName);
