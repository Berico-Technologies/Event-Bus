...
/**
 * The default implementation of the EnvelopeHandler. When an event occurs on
 * the bus, an EventEnvelopeHandler specific to that subscription is responsible
 * for attempting to execute the the EventHandler.
 * 
...
class EventEnvelopeHandler implements EnvelopeHandler {
  ...
  public EventEnvelopeHandler(AmqpEventManager amqpEventManager, EventHandler<?> eventHandler) {
    ...
    LOG.trace("EventEnvelopeHandler instantiated for EventHandler of type {}", eventHandler
            .getClass().getName());
    ...
    if (handlerMethod == null) {
      ...
      LOG.error(
              "EventHandler [{}] does not have a method called 'handleEvent', violating the contract of the EventHandler interface.",
  ...
  /**
   * An event has occurred on the Event Bus, and now it is time to handle the
   * message Envelope. We first begin by determining whether we can actually
   * handle the event with the provided EventHandler. If the event can be
   * handled, we attempt to deserialize the event and then provide it to the
   * EventHandler. The result of the EventHandler is returned.
   * 
   * @param envelope
  ...
  public EventResult handleEnvelope(Envelope envelope) {
    ...
    try {
      ...
      try {
        ...
        LOG.error(
                "Could not handle event type with the supplied EventHandler (deserialization or forname exception).",
                e);
      ...
      try {
        ...
        if (result == EventResult.Failed) {
          ...
          LOG.debug("EventHandler [{}] successfully handled the Event [{}].", eventHandler
