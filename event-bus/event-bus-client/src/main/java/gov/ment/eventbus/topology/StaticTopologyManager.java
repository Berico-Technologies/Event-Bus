...
import gov.ment.eventbus.topology.events.*;
...
public class StaticTopologyManager implements TopologyManager {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(StaticTopologyManager.class);
  ...
  private Map<String, RoutingInfo[]> topologyEventSetRegistry =
          new HashMap<String, RoutingInfo[]>();
  ...
  private void registerType(Class<?> eventType) {
    ...
    RoutingInfo route =
            new RoutingInfo(topologyExchange, RoutingInfo.ExchangeType.Topic, true, topic);
  ...
  public RoutingInfo getRoutingInfoForEvent(Class<?> eventType) {
    ...
    LOG.trace("Looking for route for event type [{}] in static topology mapper.",
