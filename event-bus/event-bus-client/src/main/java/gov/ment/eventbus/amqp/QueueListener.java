...
/**
 * Watches a Queue for new messages on a background thread, calling the
 * EnvelopeHandler when new messages arrive.
 * 
...
class QueueListener {
  ...
  /**
   * Start up an new Queue Listener bound on the supplied queue name, with the
   * provided EnvelopeHander dealing with new messages.
   * 
   * @param queueName
   ...
   *          TODO
   */
  public QueueListener(AmqpMessageBus messageBus, String queueName, Boolean queueIsDurable,
          RoutingInfo[] routes, EnvelopeHandler envelopeHandler) {
    ...
    // TODO: Need to add tests to assert that this logger name is always valid
    // (i.e. queue names with . and any
    // other illegal chars are correctly mangled.)
    LOG =
            LoggerFactory.getLogger(String.format("%s$>%s", this.getClass().getName(),
                    queueName.replace('.', '_')));
  }
  ...
  /**
   * Command the QueueListener to stop listening on the queue, thereby stopping
  ...
  public void StopListening() {
    ...
    LOG.trace("Successfully stopped consume of queue [" + queueName + "], consumerTag ["
            + consumerTag + "].");
   ...
   */
  // TODO: This method is no longer needed. We presume that StopListening now
  // blocks untill stopped.
