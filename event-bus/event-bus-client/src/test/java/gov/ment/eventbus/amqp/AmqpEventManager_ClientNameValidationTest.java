...
public class AmqpEventManager_ClientNameValidationTest {
  ...
  public void theEventManagerShouldAcceptAndWorkWithEvenInvalidNames() {
    // This test is something of a leftover. Originally it was testing that the
    // manager
    // through an exception when an invalid name was passed in. Now the manager
    // actually fixes
    // such names, so now we are really testing that no exception is thrown
    // under any
    // circumstance.
    boolean nameWasAccepted = true;
    try {
      AmqpBusFactory factory = new AmqpBusFactory();

      factory.setClientName(name);
