...
/**
 * Test fixture so assert that calling AmqpEventManger.Publish correctly invokes
 * AmqpMessaegBus.Pushlish
 */
public abstract class AmqpEventManager_PublishEnvelopeTestBase extends AmqpEventManager_TestBase {
  ...
  public void theTimeStampOfThePublishedEnvelopeShouldBeTheCurrentTime() {
    // TODO: need to insert an abstraction for Calendar into AmqpEventManager so
    // that we don't need this fuzzy
    // match.
    long delta =
            Calendar.getInstance().getTimeInMillis() - publishedEnvelope.getTimestamp().getTime();
