...
public class AmqpEventManager_NullClientNameCommandLineVariationTest extends
  ...
  public static Collection<Object[]> testObjectsToSerialize() {
    ...
    String[] commandPrefixes =
            new String[] { "", " ", "/path/to/", "c:\\windows\\path\\to\\", "url://to/",
                "/path/t.o/something.with.dots.", "c:\\windows\\path\\t.o\\something.with.dots.",
                "url://t.o/something.with.dots.", };

    String[][] commandsWithExpectedOutputs =
            new String[][] {
                ...
                { "CrazyCommand~`!@#$%^&*()_+-=[]}{';\":,<>?|",
                    "CrazyCommand~`!@#$%^&*()_+-=[]}{';\":,<>?|" },
                { StringUtils.rightPad("command-just-short-enough", 214, "x"),
                    StringUtils.rightPad("command-just-short-enough", 214, "x") },
                { StringUtils.rightPad("command-one-char-too-long", 215, "x"),
                    StringUtils.rightPad("command-one-char-too-long", 214, "x") },
                {
                    StringUtils.rightPad("@illegally-prefixed-command-just-short-enough", 214, "x"),
                    StringUtils
                            .rightPad("_@illegally-prefixed-command-just-short-enough", 215, "x") },
                {
                    StringUtils.rightPad("@illegally-prefixed-command-one-char-too-long", 215, "x"),
                    StringUtils
                            .rightPad("_@illegally-prefixed-command-one-char-too-long", 215, "x") }, };

    String[] commandSuffixes =
            new String[] { "", " ", " -switch", " -s1 arg0", " arg -s2", " /switch", " /s1 arg0",
                " arg /s2", " arg0 arg1", " arg.with.dots /s.with.dots", };
  ...
  public AmqpEventManager_NullClientNameCommandLineVariationTest(String originalCommandLine,
          String expectedFinalClientName) {
    super();
  ...
  public void clientNameShouldBeCorrectlyParsedFromCommandLineIfInitiallyNull() {
    ...
    assertTrue(String.format("Input: %s Expected: %s(%d) but was %s(%d)", originalCommandLine,
            expectedFinalClientName, expectedFinalClientName.length(),
            queueName.substring(0, queueName.length() - 37), queueName.length() - 37),
            queueName.startsWith(expectedFinalClientName));
  }
