...
public abstract class AmqpEventManager_BasicSubscribeTestBase extends AmqpEventManager_TestBase {
  ...
  public void beforeEachTest() {
    ...
    handler =
            new TestEventHandler(TestSendEvent.class, TestSendEvent2.class, TestResponseEvent.class);
  }
  ...
  public void subscribingWithoutSpecifyingAQueueNameShouldUseARandomQueueName() {
    // Really we are just asserting that multiple subscribes generate different
    // queueNames.
    ...
    verify(messageBus, times(2)).createQueue(queueNameCaptor.capture(), any(RoutingInfo[].class),
            anyBoolean());
  ...
  public void subscribingWithoutSpecifyingAQueueNameShouldUseAQueuePrefixedWithTheClientName() {
    ...
    assertTrue(String.format("Created queue name %s did not start with the expected prefix %s.",
            createdQueueName, clientName), createdQueueName.startsWith(clientName));
  }
  ...
  public void subscribingToMultipleEventsShouldCreateMultipleBindingsOnThatQueue() {
    ...
    verify(messageBus).createQueue(anyString(), routingCaptor.capture(), anyBoolean());
    ...
    for (int i = 0; i < expectedRoutes.length; i++) {
      ...
      expectedRoutes[i] =
              new RoutingInfo(expectedRoutes[i].getExchange(), expectedRoutes[i].getRoutingKey()
                      + AmqpEventManager.AMQP_ROUTE_SEGMENT_DELIMITER + getRouteSuffix());
    }
  ...
  @Test
  public void subscribingWithoutAQueueNameShouldBeginConsumingOnTheCreatedQueue()
  ...
  @Test
  public void unsubscribingASubscriptionSubscribedWithoutAQueueNameShouldDeleteTheQueue()
          throws InterruptedException {
