...
public class AmqpEventManager_SubscribeReceiveCorrectTypesTest extends AmqpEventManager_TestBase {
  ...
  public void beforeEachTest() {
    ...
    when(serializer.deserialize(bytesForMessageOfTypeTestSendEvent, TestSendEvent.class))
            .thenReturn(deserializedTestSendEvent);
    when(serializer.deserialize(bytesForMessageOfTypeTestSendEvent2, TestSendEvent2.class))
            .thenReturn(deserializedTestSendEvent2);
    when(serializer.deserialize(bytesForMessageOfTypeTestResponseEvent, TestResponseEvent.class))
            .thenReturn(deserializedTestReponseEvent);
    ...
    try {
      waitAtMost(200, TimeUnit.MILLISECONDS).untilCall(to(eventHandler.getReceivedEvents()).size(),
  ...
  @Test
  public void anEventHandlerSubscribedToMultipleEventTypesShouldReceiveEachSubscribedType()
          throws Exception {
