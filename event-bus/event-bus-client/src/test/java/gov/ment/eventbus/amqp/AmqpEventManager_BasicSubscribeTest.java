...
public class AmqpEventManager_BasicSubscribeTest extends AmqpEventManager_BasicSubscribeTestBase {
  ...
  public void aSubscriptionThatSpecifiesAQueueNameShouldBeDurable() {
    ...
    verify(messageBus, times(1)).createQueue(anyString(), any(RoutingInfo[].class),
            durrabilityCaptor.capture());
  ...
  public void subscribingWithAQueueNameShouldCauseTheRelatedQueueToBePolled()
    ...
    verify(messageBus, times(1)).beginConsumingMessages(eq("PollThisQueue"),
            any(EnvelopeHandler.class));
  }
  ...
  private LongRunningHandler performActionWhileLongRunningEventHandlerIsHandlingAnEvent(
          BackgroundActor actor) throws InterruptedException, Exception {
    ...
    // This first makes sure our test is set up properly and each thread makes
    // its call in the correct order.
  ...
  private void setupIncomingMessage() {
    ...
    // UnacceptedMessage unacceptedMessage = new
    // UnacceptedMessage(envelopeOfTypeTestSendEvent,1);
    //
  ...
  protected RoutingInfo[] getExpectedRoutes() {
    ...
    // In order to support wiretaps on queue topic X where topic X may also be
    // used for RPC
    // we must subscribe to topic X and topic X.# as the latter does not include
    // the former.
    RoutingInfo[] expectedRoutes =
            { routingInfo, routingInfo, routingInfo2, routingInfo2, returnRoutingInfo,
                returnRoutingInfo };
    for (int i = 1; i < expectedRoutes.length; i += 2) {

      expectedRoutes[i] =
              new RoutingInfo(expectedRoutes[i].getExchange(), expectedRoutes[i].getRoutingKey()
                      + AmqpEventManager.AMQP_ROUTE_SEGMENT_DELIMITER
