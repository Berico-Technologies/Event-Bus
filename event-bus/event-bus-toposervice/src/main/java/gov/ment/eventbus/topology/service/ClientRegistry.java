...
public class ClientRegistry implements Iterable<RegisterClient> {
  ...
  public void registerClient(RegisterClient registerEvent) {
    ...
    // if the client is already registered then assume that the client went down
    // and
    // has restarted and is re-registering with the topology service.
    // TODO: is this a safe assumption? What if by factory error, two
    // separate
    // services registered with the same name? Should we use client name or a
    // UUID?
