...
/**
 * A source of indirection between a specific AMQP client
 * (like RabbitMQ) and the AmqpEventManager.
...
public interface AmqpMessageBus {
	 ...
	 * @param exchange Exchange Info
	 */
	void createExchange(RoutingInfo.Exchange exchange);
	
	/**
	 * Create a Queue on the Bus.
	 * @param name Name of the Queue to be created.
	 * @param bindings Bindings between that Queue and an Exchange(s)
	...
	/**
	 * Publish a Message on the Queue, using the provided
	 * Routing Info
	 * @param route Routing Info that designates the Exchange to publish the event on.
	 * @param message Message being published
	 */
	void publish(RoutingInfo route, Envelope message);
	
	/**
	 * Pull the next message from a given queue.
	 * @param queueName Name (id) of the Queue
	 ...
	 * @param redeliverMessageLater Should the message be redelivered?
	 */
	...
	/**
	 * Represents a Message that has been received by the bus, but not accepted
	 * (meaning that we have not told the AMQP broker that the message has been 
	...
	public static class UnacceptedMessage{
		...
		/**
		 * Instantiate the Unaccepted Message with the Envelope and
		 * unique identifier.
		 * @param envelope
		...
		/**
		 * Unique Id that distinguishes this message from all others
		 * received on by this client.
		 * @return
		 */
