...
/**
 * Implementation of the Serializer interface used by the Event Bus client to
 * transform Objects into JSON Strings (and then to byte arrays) and JSON Strings
 * (originally as byte arrays) into Objects of supplied type (derived from the
 * EventHandler's "Class<? extends TEvent>[] getHandledEventTypes()" method.
 * 
...
public class GsonSerializer implements Serializer {
	...
	public byte[] serialize(Object object) {
		...
		LOG.trace("JSON Representation of [{}] is {}", 
				object.getClass().getName(), json);
	...
	public <T> T deserialize(byte[] bytes, Class<? extends T> type) {
		...
		LOG.debug("Deserializing byte[] into type {}", 
				type.getName());
